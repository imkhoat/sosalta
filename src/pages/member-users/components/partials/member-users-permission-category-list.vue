<template>
  <div
    class="member-users-permission-category-list rounded-borders overflow-hidden column justify-start items-stretch"
  >
    <div
    ...
    >
      ...
      <div class="col-12 col-md-8 col-lg-9 row justify-between items-start">
        <div>
          <q-card
            flat
            class="column"
          ...
          >
            ...
            ></q-separator>
            <q-card-section
              class="column justify-start items-stretch q-py-none"
            >
              <span class="text-disable text-caption">{{ section.title }}</span>
              <div
                v-for="(menu, menuIndex) in section?.menus.values()"
                :key="
                  sectionIndex +
                  menuIndex +
                  '__member-users-permission-card__menu'
                "
              >
                <q-input
                ...
                <q-checkbox
                  ...
                  :disable="disable"
                  :hint="
                    menu?.code ? allocationLimits.get(menu.code)?.name : ''
                  "
                  @update:model-value="
                    onChangePermission(menu, section?.relation)
                  "
                ></q-checkbox>
              </div>
            </q-card-section>
        ...
        <div class="q-mt-md">
          <q-checkbox
            v-if="module.all"
            :model-value="selectAllConfig[module.all.key]?.checked"
            :disable="disable"
            label="Select All"
            @update:model-value="onSelectAll(module)"
          ></q-checkbox>
        </div>
      </div>
    </div>
...
//data
const selectAllConfig = reactive({
  FS: {
    key: 'FS',
    checked: true,
    relatedItems: [
  ...
  CP: {
    ...
    relatedItems: [
      ...
      'CP_ALLOW_MANUAL_SHUTDOWN',
    ],
  },
  TM: {
    key: 'TM',
    checked: true,
    relatedItems: ['TM', 'TM_TEAM_SETTINGS', 'TM_TEAM_FILES'],
  },
  MU: {
    key: 'MU',
    checked: true,
    relatedItems: ['MU', 'MU_MEMBER_SETTINGS', 'MU_MEMBER_FILES'],
  },
  CA: {
    key: 'CA',
    checked: true,
  ...
  AT: {
    key: 'AT',
    checked: true,
    relatedItems: ['AT', 'AT_ACCOUNT_SETTINGS', 'AC_SAVE_BASIC_INFORMATION'],
  },
})
...
function onSelectAll(module: any) {
  selectAllConfig[module.all.key].checked =
    !selectAllConfig[module.all.key].checked
  module.all.checked = selectAllConfig[module.all.key].checked

  selectAllConfig[module.all.key].relatedItems.forEach(
    (item: PermissionCodeType) => {
      const menu = props.modelValue?.get(item)
      if (menu) {
        menu.checked = module.all.checked
      }
    }
  )
  emits('update:modelValue', props.modelValue)
...
function handleSyncSelectAllCheckbox() {
  Object.keys(selectAllConfig).forEach((item) => {
    selectAllConfig[item].checked = selectAllConfig[item].relatedItems.every(
      (it) => {
        return props.modelValue?.get(it)?.checked
      }
    )
  })
}
...
onMounted(() => {
  setTimeout(() => handleSyncSelectAllCheckbox(), 1000)
})
</script>

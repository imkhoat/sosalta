<template>
  <q-card class="recycle-bin-move-card">
    <q-form
      ref="recycleBinMoveCard"
      no-error-focus
      :autofocus="false"
      @submit="onMove"
    >
      <q-card-section>
        <q-section-header
          title="Restore file"
          sub-title="Select the folder to store the restore file"
        ></q-section-header>
      </q-card-section>
      <q-card-section>
...
const props = defineProps<{
  moveType: 'RECYCLE_BIN_PUSH_BACK_MODAL' | 'RECYCLE_BIN_PUSH_BACK_ACTIVE_MODAL'
}>()
...
const formActions = computed((): ActionProp[] => {
  return [
    ...
    },
    {
      ...
      disable: disableRestoreButton.value,
      loading: loading.value,
    },
  ]
})

const disableRestoreButton = computed(() => {
  return props.moveType === 'RECYCLE_BIN_PUSH_BACK_MODAL'
...
async function onMove() {
  try {
    ...
    if (props.moveType === 'RECYCLE_BIN_PUSH_BACK_ACTIVE_MODAL') {

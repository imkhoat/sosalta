      ...
      >
        ...
        <template v-slot:body-cell-user_role="props">
          <q-td :props="props">
            <q-chip
              outline
              :color="props.row.manager ? 'positive' : 'primary'"
              size="sm"
        ...
        <template v-slot:body-cell-user_action="props">
          <q-td :props="props">
            <div>
              <q-btn
                v-if="hasDeleteTeamMemberAuthorization"
              ...
              <q-btn
                v-if="hasAssignTeamMemberAuthorization"
                :disable="handleDisableManipulationAction(props.row)"
                flat
                round
                size="sm"
...
import { TeamMember, TEAM_MEMBER_TYPE } from 'src/core/types/team-member-types'
import { useTeamMembers } from 'src/pages/team-management/hooks/use-members'
import MemberUsersFilter from 'src/pages/team-management/components/team-members/member-users-filter.vue'
...
const { pagination, onDeleteMember, onViewUser, selectUsers } = useTeamMembers()
const { hasDeleteTeamMemberAuthorization, hasAssignTeamMemberAuthorization } =
...
const clientSearchUsers = computed(() => {
  return props.users?.filter((item) => {
    return `${item.userName}-${item.auth}-${item.type}-${item.managerString}`.includes(
      search.keyword
    )
  })
})
const filtedUser = computed(() => {
  if (search.filter === TEAM_MEMBER_TYPE.ALL) {
    return clientSearchUsers.value
  }
  return clientSearchUsers.value?.filter((item) => {
    return item.type === search.filter
  })
})
// methods
function handleDisableManipulationAction(teamMember: TeamMember) {
  return teamMember.type === TEAM_MEMBER_TYPE.OWNER && !isMainAccount.value
}
...
// hooks
onMounted(() => {
  teamMemberTableRef.value?.requestServerInteraction()
})
</script>
<style lang="scss">
.member-users-table {

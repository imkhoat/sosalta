<template>
  <div
    class="row justify-between items-center cloudpc-data-drive-filter q-gutter-y-md q-gutter-sm-x-md"
  >
    <q-btn-toggle
      v-model="filter"
      unelevated
      ...
      class="text-normal col-grow col-md-auto cloudpc-data-drive-filter__filter"
    >
      <q-btn-dropdown
        ...
        no-icon-animation
        rounded
        class="cloudpc-data-drive-filter__dropdown"
      >
        <q-list>
          <q-item
            clickable
            v-for="(user, index) in dropdownOptions"
            :key="index + '__cloudpc-data-drive-filter'"
          >
            {{ user.label }}
          </q-item>
        </q-list>
...
const firstOptions = computed(() => {
  return filterOptions.value?.slice(0, 3)
})

<template>
  <div
    class="row justify-between items-center cloudpc-machine-instances-filter q-gutter-y-md q-gutter-sm-x-md"
  >
    <q-btn-toggle
      v-if="items?.length > 0"
      v-model="filter"
      ...
      class="text-normal col-grow col-md-auto cloudpc-machine-instances-filter__filter"
    >
      <q-btn-dropdown
        ...
        no-icon-animation
        rounded
        class="cloudpc-machine-instances-filter__dropdown"
      >
        <q-list>
          <q-item
            clickable
            v-for="(user, index) in dropdownOptions"
            :key="index + '__cloudpc-machine-instances-filter'"
          >
            {{ user.label }}
          </q-item>
        </q-list>
...
<style lang="scss">
.cloudpc-machine-instances-filter {

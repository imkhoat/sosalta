<template>
  <q-card
    flat
    outlined
    class="rounded-borders font-opensan"
  >
    <q-card-section
      class="column justify-start items-stretch q-gutter-y-md rounded-borders bg-purple-1"
    >
      ...
      <span class="text-body2 text-weight-bold">Total cashs</span>
      ...
      <div>
        <span>Plus {{ totalCoinFormated }} coins</span>
        <span>({{ totalCoinByMoney }})</span>
      </div>
    </q-card-section>
  </q-card>
</template>
...
const totalCoinFormated = computed(() => {
  return props.coin?.toLocaleString()
})
